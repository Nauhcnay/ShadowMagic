...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, attention = False, wgan = False, activation = None):
        super().__init__() 
        if attention:   
            self.maxpool_conv = nn.Sequential(
                nn.MaxPool2d(2),
                DoubleConv(in_channels, out_channels, wgan = wgan, activation = activation),
...
class DownDilated(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, wgan = False, activation = None):
        super().__init__()
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleDilatedConv(in_channels, out_channels, wgan = wgan, activation = activation)
        )
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True, wgan = False, activation = None):
        super().__init__()

        # if bilinear, use the normal convolutions to reduce the number of channels
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.conv = DoubleConv(in_channels, out_channels, in_channels // 2, wgan = wgan, activation = activation)
        else:
            self.up = nn.ConvTranspose2d(in_channels , in_channels // 2, kernel_size=2, stride=2)
            self.conv = DoubleConv(in_channels, out_channels, wgan = wgan, activation = activation)
...
class DownResNet(nn.Module):
    def __init__(self, in_channels, out_channels, attn = False, drop_out = -1, wgan = False, activation = None):
        super().__init__()
        if attn:
            self.layers = nn.Sequential(
                nn.MaxPool2d(2),
                ResBlock(in_channels, out_channels, drop_out = drop_out, wgan = wgan, activation = activation),
                LayerAttention(out_channels),
                SpatialAttention(out_channels)
            )
        else:
...
class UpResNet(nn.Module):
    def __init__(self, in_channels, out_channels, wgan = False, activation = None):
        super().__init__()
        self.up = nn.Upsample(scale_factor = 2, mode = 'bilinear', align_corners = True)
        self.res_block = ResBlock(in_channels, out_channels, wgan = wgan, activation = activation)
...
class DilatedConvResNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None, wgan = False, activation = None):
        ...
        if not mid_channels:
            mid_channels = out_channels
        self.double_conv = nn.Sequential(
            ResBlock(in_channels, mid_channels, kernel_size=3, dilation=2, wgan = wgan, activation = activation),
            ResBlock(mid_channels, out_channels, kernel_size=3, dilation=2, wgan = wgan, activation = activation)
        )
